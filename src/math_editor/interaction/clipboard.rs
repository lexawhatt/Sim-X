...
use crate::math_editor::{formula::Formula, state::MathState};
...
pub(crate) struct ClipboardRequest {
    ...
    pub row: usize,
    pub(in crate::math_editor) original: Formula,
    pub(in crate::math_editor) fragment: Option<Formula>,
    pub text: String,
    epoch: u64,
}
impl MathState {
    ...
    pub(crate) fn complete_clipboard(&mut self, result: Result<String, String>) {
        ...
        let fragments = if request.action == ClipboardAction::Paste {
        ...
        if fragments.len() > 1 {
            self.paste_expressions(request.row, fragments);
            return;
        }
...
/// Newlines separate complete expressions, never slots within one formula.
/// Validate every nonblank line before applying anything to the document.
fn parse_expressions(text: &str) -> Result<Vec<Formula>, String> {
    text.lines()
        .enumerate()
        .filter(|(_, line)| !line.trim().is_empty())
        .map(|(index, line)| {
            Formula::from_text(line.trim()).map_err(|error| format!("Line {}: {error}", index + 1))
        })
        .collect()
}
