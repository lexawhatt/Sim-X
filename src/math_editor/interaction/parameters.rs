...
pub(in crate::math_editor) struct Range {
    pub low: f64,
    pub high: f64,
}
#[derive(Clone, Copy)]
pub(in crate::math_editor) struct Parameter {
    pub name: char,
    pub value: f64,
    pub range: Range,
}
pub(in crate::math_editor) struct Playback {
    pub row: usize,
    pub phase: f64,
    pub before: Document,
}
...
impl MathState {
    pub(in crate::math_editor) fn scan_parameters(&mut self) {
    ...
    pub(in crate::math_editor) fn add_parameter(&mut self, name: char) {
        self.stop_parameter_motion();
        let Ok(formula) = Formula::from_text(&format!("{name}=1")) else {
            return;
        };
        let before = self.document.clone();
        let index = self.document.fields.len();
        self.insert_formula(index, formula);
        self.focus_expression(index, true);
        self.remember(before);
    }
    pub(in crate::math_editor) fn set_parameter(&mut self, row: usize, value: f64) {
    ...
    pub(in crate::math_editor) fn move_slider(&mut self, row: usize, x: f32, track: layout::Rect) {
    ...
    pub(in crate::math_editor) fn toggle_parameter(&mut self, row: usize) {
    ...
    pub(in crate::math_editor) fn advance_parameters(&mut self, dt: f64) {
    ...
    pub(in crate::math_editor) fn stop_parameter_motion(&mut self) {
        let before = self
            .parameter_drag
            .take()
            .map(|(_, d)| d)
    ...
    pub(in crate::math_editor) fn commit_range(&mut self) {
