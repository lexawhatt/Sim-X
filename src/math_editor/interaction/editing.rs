...
#[derive(Default)]
pub(in crate::math_editor) struct EditingSession {
...
impl EditingSession {
    pub(in crate::math_editor) fn reset(&mut self) {
        *self = Self::default();
    }
    ...
    pub(in crate::math_editor) fn coalesce(
        &mut self,
        before: &Document,
        after: &Document,
        focus: Option<usize>,
...
impl MathState {
    pub(in crate::math_editor) fn editor_key(&mut self, key: PhysicalKeyCode) {
        use PhysicalKeyCode::*;
        ...
        let kind = if ctrl || self.confirm_back || self.range_edit.is_some() {
            ...
            match key {
                ...
                _ => interaction::character(key, shift)
        ...
        interaction::key_press(self, key);
    ...
    pub(in crate::math_editor) fn observe_editor_key(&mut self, key: PhysicalKeyCode, down: bool) {
        use PhysicalKeyCode::*;
        if !down {
        ...
        let eligible = self.editing.blocked_until_release != Some(key)
            ...
            ) || interaction::character(key, self.shifts.iter().any(|v| *v))
                .is_some_and(|ch| ch.is_ascii_alphanumeric() || ch == '.'));
        self.editing.held = eligible.then_some((key, 0.45));
    }

    pub(in crate::math_editor) fn advance_editing(&mut self, dt: f32) -> bool {
