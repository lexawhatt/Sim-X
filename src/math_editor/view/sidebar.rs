...
use crate::math_editor::{
    formula,
    scene::{integrals, style},
    state::MathState,
    view::{
        assets::Fonts,
        drawing::*,
        layout::{self, Layout, Rect, Target},
    },
};
...
pub(in crate::math_editor) fn draw(
    ...
    for (i, row) in layout.rows.iter().enumerate() {
            ...
            .filter(|_| !state.stale)
        {
            if state.spatial.blend > 0.0
                && let Some(hint) = interpretation::xy_curve_hint(&state.document.fields[i], result)
            {
            ...
            if let Some(picture) = &result.integral_plot {
                let baseline =
                    row.y + row.h - layout::parameter_extra(state, i, layout.sidebar) - 34.0;
                let x = if picture.bands.is_empty() {
                    ...
                    out.text(
                        ...
                        [69.0, baseline],
                        2,
                        integrals::negative_color(style),
                        layout.list,
                    );
                    86.0
                };
            ...
            if let Some(message) = &result.diagnostic {
                out.text(
                    ...
                    [
                        52.0,
                        row.y + row.h - layout::parameter_extra(state, i, layout.sidebar) - 8.0,
                    ],
                    2,
    ...
    for (index, rect) in layout.fields.iter().copied().enumerate() {
        ...
        if state.document.fields[index].select_all && state.focus == Some(index) {
            ...
            && let (Some(a), Some(b)) = (
                formula.stop(formula::Caret { row, index: start }),
                formula.stop(formula::Caret { row, index: end }),
            )
        {
...
pub(in crate::math_editor) mod interpretation {
    ...
    pub(in crate::math_editor) fn xy_curve_hint(
        formula: &Formula,
        result: &RowPlot,
    ) -> Option<&'static str> {
