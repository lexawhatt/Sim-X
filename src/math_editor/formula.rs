...
#[cfg(test)]
#[path = "formula_editing_tests.rs"]
mod editing_tests;
impl Atom {
    pub(super) fn children(&self) -> Vec<usize> {
        match self {
            Self::Symbol(_) => vec![],
...
impl Formula {
    ...
    pub(super) fn product_before(&self, row: usize, index: usize) -> ProductBoundary {
    ...
    pub(super) fn row(&mut self, atoms: Vec<Atom>) -> usize {
