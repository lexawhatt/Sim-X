...
use super::{
    assets::Fonts, drawing::*, graph_style::GraphStyle, layout::Layout, spatial::Projector,
    state::MathState,
};
...
pub(super) fn draw(out: &mut Scene, state: &MathState, layout: &Layout, fonts: &Fonts) {
    ...
    super::axis_view::transition(out, state, layout, &projector, fonts);
    let wireframe = super::spatial_clip::Bounds::for_view(state.camera, rect)
        .map(|bounds| super::wireframe_view::Wireframe::new(&projector, bounds, rect));
        ...
        for (index, row) in plot.rows.iter().enumerate() {
            ...
            if let Some(picture) = &row.integral_plot {
                ...
                if planar > 0.0 && !state.stale && row.scalar.is_some() && !row.pending {
                    ...
                    super::integral_view::fill_plane(
                        out,
                        picture,
                        fill,
                        rect,
    ...
    let mut labels = super::point_labels::PointLabels::new(out, layout, fonts);
    ...
    if let Some(backing) = super::layout::Rect::new(
        rect.x + 10.0,
        rect.y + 12.0,
