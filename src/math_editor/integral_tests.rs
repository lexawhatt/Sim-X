use super::super::{
    ...
    integral_plot::IntegralPlot,
    integral_view,
    layout::{Camera, Rect},
    worker::{Plot, Request, RowPlot},
};
use super::*;
...
fn crossings_and_reversed_limits_keep_signed_and_geometric_area_distinct() {
    ...
    assert!(
        crossing
            .panels
            .iter()
            .any(|p| p.color.red() == integral_view::negative_color(GraphStyle::new(0)).red())
    );
...
fn view_only_resampling_reuses_scalars_but_edit_or_cancel_invalidates_them() {
    let mut cache = super::super::worker_calculus::CalculationCache::default();
...
fn native_input_difference_displays_curves_then_signed_fill_and_follows_edits() -> LogicResult {
    ...
    let mut scene = Scene::default();
    let fonts = runner
        .resource::<super::super::assets::Fonts>()
        .unwrap()
        .clone();
    super::super::graph_view::draw(&mut scene, s, &layout, &fonts);
...
fn integral_region_cpu_preview() -> LogicResult {
    ...
    let mut scene = Scene::default();
    super::super::graph_view::draw(
        &mut scene,
        &state,
        &layout,
        runner.resource::<super::super::assets::Fonts>().unwrap(),
    );
