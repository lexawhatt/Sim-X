...
use super::worker_calculus::{Calculation, CalculationCache};
use sim_math::{
    Expression, Integral, IntegralKind, IntegrationJob,
    geometry::Point,
...
pub(super) struct RowPlot {
    ...
    pub integral_plot: Option<super::integral_plot::IntegralPlot>,
}
#[derive(Clone, Default)]
        ...
        let result = (|| {
            match Statement::parse(source, &variables)? {
                ...
                Statement::Implicit3d(relation) => {
                    super::worker_spatial::implicit(row, &relation, request, alive)?
                }
                Statement::Surface(f) => {
                    if request.spatial {
                        let bounds =
                            super::spatial_clip::Bounds::from_xy(request.lower, request.upper)
                ...
                Statement::Integral {
                    ...
                    if !graph_result {
                        row.integral_plot = Some(super::integral_plot::IntegralPlot::prepare(
...
mod tests {
    ...
    fn structured_integral_difference_waits_for_calculate_then_graphs_scalar() {
        let formula =
            super::super::formula::Formula::from_text("y=integral(0,1,x^2)-integral(0,1,x)")
                .unwrap();
