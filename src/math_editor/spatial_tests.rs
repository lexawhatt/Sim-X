...
fn integral_bands_fade_near_2d_and_are_absent_from_the_settled_orbit_view() -> LogicResult {
    ...
    let fonts = runner.resource::<super::super::assets::Fonts>().unwrap();
    ...
    for blend in [0.0, 0.1, 0.2, 1.0, 0.0] {
        ...
        super::super::graph_view::draw(&mut scene, &view, &layout, fonts);
        alpha.push(
            scene
                .panels
                .iter()
...
fn native_3d_switch_orbit_and_return_preserve_document() -> LogicResult {
    ...
    super::super::visual_probe::export(&scene, l.canvas);
    assert!(
