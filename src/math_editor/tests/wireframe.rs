...
use crate::math_editor::{
    compute, formula,
    scene::{Spatial, clip::Bounds, style::GraphStyle, wireframe::Wireframe},
    state,
    tests::visual_probe,
    view::{
        assets::Fonts,
...
fn clipped_saddle_cpu_preview_and_protected_labels() -> LogicResult {
    ...
    state.plot = Some(compute::Plot {
        rows: vec![compute::RowPlot {
            surface,
            ..Default::default()
        }],
    });
    let mut out = Scene::default();
    graph::draw(&mut out, &state, &layout, fonts);
    ...
    visual_probe::export(&out, layout.canvas);
    Ok(())
}
