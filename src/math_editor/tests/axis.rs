...
use crate::math_editor::{
    scene::axes::{self, spatial as spatial_axes, ticks},
    state,
    tests::visual_probe,
    view::{
        assets::Fonts,
...
fn adaptive_tick_density_is_zero_anchored_and_labels_keep_precision() {
    for scale in [1e-12, 0.0008, 0.02, 8.1, 62.0, 422.0, 1e9, 1e15] {
        let spacing = ticks::Spacing::new(scale, 30.0).unwrap();
        assert!(spacing.major * scale >= 88.0 - 1e-8);
        let extent = 1450.0 / scale;
        let ticks = ticks::ticks(-extent * 0.5, extent * 0.5, spacing.major);
        assert!(ticks.len() <= 18);
        assert!(ticks.contains(&0.0));
        for v in ticks {
            assert!((v / spacing.major - (v / spacing.major).round()).abs() < 1e-8);
        }
        assert!(ticks::ticks(-extent * 0.5, extent * 0.5, spacing.minor).len() <= 85);
    }
    assert_eq!(ticks::label(-0.0, 0.1), "0");
    assert_eq!(ticks::label(0.2, 0.1), "0.2");
    assert_eq!(ticks::label(20.0, 10.0), "20");
    assert_ne!(ticks::label(1e9, 0.2), ticks::label(1e9 + 0.2, 0.2));
    assert_ne!(ticks::label(1e-7, 2e-8), ticks::label(1.2e-7, 2e-8));
    for scale in [0.0, -1.0, f64::NAN, f64::INFINITY] {
        assert!(ticks::Spacing::new(scale, 20.0).is_none());
    }
    assert!(ticks::ticks(1e30, 1e30 + 1e20, 1.0).is_empty());
}
...
fn zoomed_grid_and_xyz_labels_do_not_overlap_or_flood_the_view() -> LogicResult {
    ...
    for size in [[900.0, 640.0], [1280.0, 800.0], [1920.0, 1080.0]] {
        ...
        for scale in [8.1, 62.0, 422.0, 0.00001, 1e9] {
            ...
            axes::plane(&mut scene, &state, &layout, fonts);
            ...
            let mut scene = Scene::default();
            spatial_axes::draw(&mut scene, &state, &layout, &projector, fonts);
            assert_label_separation(&scene, fonts, layout.canvas);
...
fn axis_layers_crossfade_without_toggle_pop_or_unbounded_line_counts() -> LogicResult {
    ...
    for blend in [0.0, 0.01, 0.15, 0.5, 0.85, 0.99, 1.0] {
        ...
        let projector = state.spatial.projector(state.camera, layout.canvas)?;
        let mut scene = Scene::default();
        axes::transition(&mut scene, &state, &layout, &projector, fonts);
        ...
        if blend == 0.0 {
            ...
            axes::plane(&mut planar, &state, &layout, fonts);
