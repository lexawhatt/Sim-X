...
use super::{
    drawing::Scene,
    graph_style::{self, GraphStyle},
    integral_plot::{IntegralPlot, Section},
    layout::Rect,
};
use sim_logic::prelude::Color;
use sim_math::geometry::Point;

pub(super) fn negative_color(style: GraphStyle) -> Color {
    graph_style::color(style.color + 2)
}
    ...
    for (index, points) in picture.curves.iter().enumerate() {
        ...
        for p in points.iter().chain(std::iter::once(&None)) {
            if let Some(p) = p.and_then(&project) {
                ...
                curve.draw_path(out, &super::curve_display::simplify(&run, 0.35), rect);
                run.clear();
            }
        }
    }
