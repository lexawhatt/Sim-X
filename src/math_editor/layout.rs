...
impl Layout {
    pub fn for_state(viewport: LogicalViewport, state: &super::state::MathState) -> Self {
        ...
        if state.keypad && state.alphabet {
            super::keyboard_layout::alphabet(&mut controls, key_start, keypad_top);
            ...
            super::keyboard_layout::numeric(&mut controls, key_start, keypad_top);
        }
...
pub(super) fn parameter_extra(state: &super::state::MathState, i: usize, sidebar: f32) -> f32 {
    if state.parameters.get(i).is_some_and(Option::is_some) {
        40.0
    } else {
        let columns = ((sidebar - 62.0) / 78.0).floor() as usize;
